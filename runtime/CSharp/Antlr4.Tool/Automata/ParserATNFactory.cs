...
namespace Antlr4.Automata
{
    ...
    public class ParserATNFactory : ATNFactory
    {
        ...
        [NotNull] protected readonly IList<Tuple<Rule, ATNState, ATNState>> preventEpsilonClosureBlocks =
            new List<Tuple<Rule, ATNState, ATNState>>();

        [NotNull] protected readonly IList<Tuple<Rule, ATNState, ATNState>> preventEpsilonOptionalBlocks =
            new List<Tuple<Rule, ATNState, ATNState>>();
        ...
        public ParserATNFactory([NotNull] Grammar g)
        {
            ...
            atn = new ATN(atnType, maxTokenType);
        }
        ...
        public virtual ATN CreateATN()
        {
            ...
            foreach (Tuple<Rule, ATNState, ATNState> pair in preventEpsilonClosureBlocks)
            {
                LL1Analyzer analyzer = new(atn);
                ...
                if (lookahead.Contains(TokenConstants.Epsilon))
                {
            ...
            foreach (Tuple<Rule, ATNState, ATNState> pair in preventEpsilonOptionalBlocks)
            {
                int bypassCount = 0;
                for (int i = 0;
                    i < pair.Item2.NumberOfTransitions;
                    i++)
                {
                    ...
                    LL1Analyzer analyzer = new(atn);
                    if (analyzer.Look(startState, pair.Item3, PredictionContext.EmptyLocal).Contains(TokenConstants.Epsilon))
                    {
        ...
        public virtual Handle Sempred([NotNull] PredAST pred)
        {
            ...
            {
                int precedence = Int32.Parse(pred.GetOptionString(LeftRecursiveRuleTransformer.PRECEDENCE_OPTION_NAME));
        ...
        public virtual Handle Action([NotNull] ActionAST action)
        {
            ...
            ActionTransition a = new(right, currentRule.index);
        ...
        public virtual Handle _RuleRef([NotNull] GrammarAST node)
        {
            ...
            {
                precedence = Int32.Parse(((GrammarASTWithOptions) node).GetOptionString(LeftRecursiveRuleTransformer.PRECEDENCE_OPTION_NAME));
            }

            RuleTransition call = new(start, r.index, precedence, right);
        ...
        protected virtual Handle MakeBlock([NotNull] BlockStartState start, [NotNull] BlockAST blkAST, [NotNull] IList<Handle> alts)
        {
            ...
            foreach (Handle alt in alts)
            {
                ...
                TailEpsilonRemover opt = new(atn);
                opt.Visit(alt.left);
            }

            Handle h = new(start, end);
        ...
        [return: NotNull]
        public virtual ATNState NewState([AllowNull] GrammarAST node)
        {
