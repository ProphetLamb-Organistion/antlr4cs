...
namespace Antlr4.Tool
{
    public class LeftRecursiveRule : Rule
    {
         ...
         */
        public override IDictionary<string, IList<Tuple<int, AltAST>>> GetAltLabels()
        {
            IDictionary<string, IList<Tuple<int, AltAST>>> labels = new Dictionary<string, IList<Tuple<int, AltAST>>>();
            IDictionary<string, IList<Tuple<int, AltAST>>> normalAltLabels = base.GetAltLabels();
            if (normalAltLabels != null)
            {
                foreach (KeyValuePair<string, IList<Tuple<int, AltAST>>> pair in normalAltLabels)
                {
            ...
            if (recPrimaryAlts != null)
            {
                foreach (LeftRecursiveRuleAltInfo altInfo in recPrimaryAlts)
                {
                    if (altInfo.altLabel != null)
                    {
                        IList<Tuple<int, AltAST>> pairs;
                        if (!labels.TryGetValue(altInfo.altLabel, out pairs) || pairs == null)
                        {
                            pairs = new List<Tuple<int, AltAST>>();
