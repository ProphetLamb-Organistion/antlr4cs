...
namespace Antlr4.Tool
{
    ...
    public class GrammarParserInterpreter : ParserInterpreter
    {
        ...
        {
            BitSet track = new(atn.states.Count);
         ...
         * *
         * <p>
         *     So, there are two decisions associated with picking the outermost alt.
        ...
        protected override int VisitDecisionState(DecisionState p)
        {
            ...
            if (p.NumberOfTransitions > 1)
            {
                ...
                if (p.decision == overrideDecision &&
            ...
            int stopIndex)
        {
            ...
            for (int alt = 1;
                alt <= decisionState.Transitions.Length;
                alt++)
            {
                // re-parse entire input for all ambiguous alternatives
                // (don't have to do first as it's been parsed, but do again for simplicity
                //  using this temp parser.)
                BailButConsumeErrorStrategy errorHandler = new();
        ...
        public class BailButConsumeErrorStrategy : DefaultErrorStrategy
        {
            ...
            public override IToken RecoverInline(Parser recognizer)
            {
                ...
                InputMismatchException e = new(recognizer);
