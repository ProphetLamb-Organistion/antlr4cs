...
namespace Antlr4.Misc
{
    ...
    public static class Utils
    {
        ...
        private static object[] ints = new object[INTEGER_POOL_MAX_VALUE + 1];
         ...
         */
        public static IList<To> Select<From, To>(IList<From> list, Func<From, To> selector)
        {
            if (list == null)
            {
                return null;
        ...
        public static int IndexOf<T>(IList<T> elems, Predicate<T> match)
        {
            for (int i = 0;
                i < elems.Count;
                i++)
        ...
        public static int LastIndexOf<T>(IList<T> elems, Predicate<T> match)
        {
            for (int i = elems.Count - 1;
                i >= 0;
                i--)
            {
                if (match(elems[i]))
