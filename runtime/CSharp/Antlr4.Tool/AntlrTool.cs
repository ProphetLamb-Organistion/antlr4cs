...
namespace Antlr4
{
    public class AntlrTool
    {
        ...
        {
            new(nameof(outputDirectory), "-o", OptionArgType.STRING, "specify output directory where all output is generated"),
            new(nameof(libDirectory), "-lib", OptionArgType.STRING, "specify location of grammars, tokens files"),
            new(nameof(generate_ATN_dot), "-atn", "generate rule augmented transition network diagrams"),
            new(nameof(grammarEncoding), "-encoding", OptionArgType.STRING, "specify grammar file encoding; e.g., euc-jp"),
            new(nameof(msgFormat), "-message-format", OptionArgType.STRING, "specify output style for messages in antlr, gnu, vs2005"),
            new(nameof(longMessages), "-long-messages", "show exception details when available for errors and warnings"),
            new(nameof(gen_listener), "-listener", "generate parse tree listener (default)"),
            new(nameof(gen_listener), "-no-listener", "don't generate parse tree listener"),
            new(nameof(gen_visitor), "-visitor", "generate parse tree visitor"),
            new(nameof(gen_visitor), "-no-visitor", "don't generate parse tree visitor (default)"),
            new(nameof(genPackage), "-package", OptionArgType.STRING, "specify a package/namespace for the generated code"),
            new(nameof(gen_dependencies), "-depend", "generate file dependencies"),
            new("", "-D<option>=value", "set/override a grammar-level option"),
            new(nameof(warnings_are_errors), "-Werror", "treat warnings as errors"),
            new(nameof(launch_ST_inspector), "-XdbgST", "launch StringTemplate visualizer on generated code"),
            new(nameof(ST_inspector_wait_for_close), "-XdbgSTWait", "wait for STViz to close before continuing"),
            new(nameof(force_atn), "-Xforce-atn", "use the ATN simulator for all predictions"),
            new(nameof(log), "-Xlog", "dump lots of logging info to antlr-timestamp.log")
        };
         ...
         */
        private readonly DefaultToolListener defaultListener;
        ...
        public IDictionary<string, string> grammarOptions;
        ...
        private readonly IList<ANTLRToolListener> listeners = new List<ANTLRToolListener>();
        ...
        static AntlrTool()
        {
            Assembly assembly = typeof(AntlrTool).GetTypeInfo().Assembly;
        ...
        public TextWriter ConsoleOut
        {
            ...
            set => _consoleOut = value ?? throw new ArgumentNullException(nameof(value));
        }
        ...
        public TextWriter ConsoleError
        {
            ...
            set => _consoleError = value ?? throw new ArgumentNullException(nameof(value));
        }

        public static int Main(string[] args)
        {
            AntlrTool antlr = new(args);
            if (args.Length == 0)
            {
                antlr.Help();
                return 0;
        ...
        public virtual void Process(Grammar g, bool gencode)
        {
            ...
            GrammarTransformPipeline transform = new(g, this);
            transform.Process();
        ...
        public virtual bool CheckForRuleIssues(Grammar g)
        {
            ...
            UndefChecker chk = new(this, g, ruleToAST);
        ...
        public virtual GrammarRootAST ParseGrammar(string fileName)
        {
            try
            {
                ...
                ANTLRStringStream @in = new(fileChars, fileChars.Length, fileName);
        ...
        public virtual Grammar LoadImportedGrammar(Grammar g, GrammarAST nameNode)
        {
            ...
            if (!importedGrammars.TryGetValue(name, out imported) || imported == null)
            {
                ...
                char[] fileChars = fileContent.ToCharArray();
                ANTLRStringStream @in = new(fileChars, fileChars.Length, importedFile);
        ...
        public virtual GrammarRootAST Parse(string fileName, ICharStream @in)
        {
            try
            {
                GrammarASTAdaptor adaptor = new(@in);
                ToolANTLRLexer lexer = new(@in, this);
                CommonTokenStream tokens = new CommonTokenStream(lexer);
                lexer.tokens = tokens;
                ToolANTLRParser p = new(tokens, this);
        ...
        {
            DOTGenerator dotGenerator = new(g);
        ...
        public virtual void Help()
        {
            ...
            foreach (Option o in optionDefs)
            {
                ...
                string s = $" {name} {o.description}";
                Info(s);
            }
        }

        public virtual void Log([AllowNull] string component, string msg)
        {
