...
namespace Antlr4.Runtime.Atn
{
    ...
    public class ATNDeserializer
    {
        ...
        [NotNull] private readonly ATNDeserializationOptions deserializationOptions;
        ...
        public ATNDeserializer([AllowNull] ATNDeserializationOptions deserializationOptions)
        {
        ...
        public virtual ATN Deserialize([NotNull] char[] data)
        {
            ...
            {
                string reason = $"Could not deserialize ATN with version {version} (expected {SerializedVersion}).";
            ...
            {
                string reason = $"Could not deserialize ATN with UUID {uuid} (expected {SerializedUuid} or a legacy UUID).";
                throw new NotSupportedException(reason);
            }
            ...
            var returnTransitionsSet = new HashSet<Tuple<int, int, int>>();
            var returnTransitions = new List<Tuple<int, int, int>>();
            foreach (ATNState state_1 in atn.states)
            {
                ...
                for (int i_10 = 0;
                    i_10 < state_1.NumberOfTransitions;
                    i_10++)
                {
                    ...
                    Tuple<int, int, int> returnTransition = Tuple.Create(ruleTransition.target.ruleIndex, ruleTransition.followState.stateNumber, outermostPrecedenceReturn);
            ...
            {
                EpsilonTransition transition = new(atn.states[returnTransition.Item2], returnTransition.Item3);
            ...
            foreach (ATNState state_2 in atn.states)
            {
                ...
                if (state_2 is PlusLoopbackState)
                {
                    ...
                    for (int i_10 = 0;
                        i_10 < loopbackState.NumberOfTransitions;
                        i_10++)
                    {
            ...
            if (atn.grammarType == ATNType.Lexer)
            {
                ...
                else
                {
                    ...
                    var legacyLexerActions = new List<ILexerAction>();
                    foreach (ATNState state_3 in atn.states)
                    {
                        for (int i_10 = 0;
                            i_10 < state_3.NumberOfTransitions;
                            i_10++)
                        {
                            ...
                            LexerCustomAction lexerAction = new(ruleIndex, actionIndex);
            ...
            if (deserializationOptions.GenerateRuleBypassTransitions && atn.grammarType == ATNType.Parser)
            {
                ...
                for (int i_10 = 0;
                    i_10 < atn.ruleToStartState.Length;
                    i_10++)
                {
                ...
                for (int i_13 = 0;
                    i_13 < atn.ruleToStartState.Length;
                    i_13++)
                {
        ...
        private static int InlineSetRules(ATN atn)
        {
            ...
            for (int i = 0;
                i < atn.ruleToStartState.Length;
                i++)
            {
                RuleStartState startState = atn.ruleToStartState[i];
                ATNState middleState = startState;
                while (middleState.OnlyHasEpsilonTransitions && middleState.NumberOfOptimizedTransitions == 1 &&
                       middleState.GetOptimizedTransition(0).TransitionType == TransitionType.Epsilon)
                {
                ...
                if (matchTransition.IsEpsilon || !matchTarget.OnlyHasEpsilonTransitions || matchTarget.NumberOfOptimizedTransitions != 1 ||
            ...
            for (int stateNumber = 0;
                stateNumber < atn.states.Count;
                stateNumber++)
            {
                ...
                for (int i_1 = 0;
                    i_1 < state.NumberOfOptimizedTransitions;
                    i_1++)
                {
        ...
        private static int CombineChainedEpsilons(ATN atn)
        {
            ...
            foreach (ATNState state in atn.states)
            {
                    ...
                    i++)
                {
                    ...
                    if (transition.TransitionType != TransitionType.Epsilon || ((EpsilonTransition) transition).OutermostPrecedenceReturn != -1 ||
                        intermediate.StateType != StateType.Basic || !intermediate.OnlyHasEpsilonTransitions)
                    {
                        if (optimizedTransitions != null)
                        {
                            optimizedTransitions.Add(transition);
                    ...
                    for (int j_2 = 0;
                        j_2 < intermediate.NumberOfOptimizedTransitions;
                        j_2++)
                    {
        ...
        private static int OptimizeSets(ATN atn, bool preserveOrder)
        {
            ...
            foreach (DecisionState decision in decisions)
            {
                ...
                for (int i = 0;
                    i < decision.NumberOfOptimizedTransitions;
                    i++)
                {
                ...
                for (int i_1 = 0;
                    i_1 < decision.NumberOfOptimizedTransitions;
                    i_1++)
                {
                ...
                for (int i_2 = 0;
                    i_2 < setTransitions.GetIntervals().Count;
                    i_2++)
                {
        ...
        private static bool TestTailCall(ATN atn, RuleTransition transition, bool optimizedPath)
        {
            ...
            BitSet reachable = new(atn.states.Count);
        ...
        protected internal virtual Transition EdgeFactory([NotNull] ATN atn, TransitionType type, int src, int trg, int arg1, int arg2, int arg3, IList<IntervalSet> sets)
        {
            ...
            switch (type)
            {
                ...
                {
                    RuleTransition rt = new((RuleStartState) atn.states[arg1], arg2, arg3, target);
                    return rt;
                }

                case TransitionType.Predicate:
                {
                    PredicateTransition pt = new(target, arg1, arg2, arg3 != 0);
                    return pt;
                }
                ...
                {
                    ActionTransition a = new(target, arg1, arg2, arg3 != 0);
                    return a;
                }
