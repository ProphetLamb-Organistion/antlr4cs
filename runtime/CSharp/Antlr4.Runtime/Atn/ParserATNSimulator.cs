...
#pragma warning disable 0162 // Unreachable code detected
        ...
        [MaybeNull] protected internal readonly Parser parser;
        ...
        public ParserATNSimulator([AllowNull] Parser parser, [NotNull] ATN atn)
            : base(atn)
        {
            this.parser = parser;
        }
        ...
        public virtual int AdaptivePredict([NotNull] ITokenStream input, int decision, [AllowNull] ParserRuleContext outerContext)
        {
            return AdaptivePredict(input, decision, outerContext, false);
        }

        public virtual int AdaptivePredict([NotNull] ITokenStream input, int decision, [AllowNull] ParserRuleContext outerContext, bool useContext)
        {
            ...
            userWantsCtxSensitive = useContext || predictionMode != PredictionMode.Sll && outerContext != null && !atn.decisionToState[decision].sll;
        ...
        protected internal virtual SimulatorState GetStartState([NotNull] DFA dfa, [NotNull] ITokenStream input, [NotNull] ParserRuleContext outerContext, bool useContext)
        {
            ...
            Debug.Assert(outerContext != null);
            DFAState s0;
            if (dfa.IsPrecedenceDfa)
            {
            ...
            while (remainingContext != null && s0 != null && s0.IsContextSensitive)
            {
                ...
                {
                    Debug.Assert(s0 == null || !s0.IsContextSensitive);
                }
                else
                {
        ...
        protected internal virtual int ExecDFA([NotNull] DFA dfa, [NotNull] ITokenStream input, int startIndex, [NotNull] SimulatorState state)
        {
            ...
            while (true)
            {
                if (state.useContext)
                {
                    while (s.IsContextSymbol(t))
                    {
                        ...
                        {
                            // fail over to ATN
                            SimulatorState initialState = new(state.outerContext, s, state.useContext, remainingOuterContext);
                            return ExecATN(dfa, input, startIndex, initialState);
                        }

                        Debug.Assert(remainingOuterContext != null);
                ...
                Debug.Assert(!IsAcceptState(s, state.useContext));
                ...
                if (target == null)
                {
                    if (dfa_debug && t >= 0)
                    {
                        Console.Out.WriteLine("no edge for " + parser.Vocabulary.GetDisplayName(t));
                    }

                    int alt;
                    SimulatorState initialState = new(outerContext, s, state.useContext, remainingOuterContext);
                ...
                if (target == Error)
                {
                    SimulatorState errorState = new(outerContext, s, state.useContext, remainingOuterContext);
            ...
            if (!state.useContext && s.configs.ConflictInformation != null)
            {
                if (dfa.atnStartState is DecisionState)
                {
                    ...
                    else
                    {
                        ...
                        {
                            SimulatorState conflictState = new(outerContext, s, state.useContext, remainingOuterContext);
            ...
            if (predicates_1 != null)
            {
                ...
                BitSet alts = EvalSemanticContext(predicates_1, outerContext, reportAmbiguities && predictionMode == PredictionMode.LlExactAmbigDetection);
        ...
        protected internal virtual int ExecATN([NotNull] DFA dfa, [NotNull] ITokenStream input, int startIndex, [NotNull] SimulatorState initialState)
        {
            ...
            PredictionContextCache contextCache = new();
            while (true)
            {
                // while more work
                ...
                Debug.Assert(D.IsAcceptState || D.Prediction == ATN.InvalidAltNumber);
                // conflicted => accept state
                Debug.Assert(D.IsAcceptState || D.configs.ConflictInformation == null);
                if (IsAcceptState(D, useContext))
                {
                    ...
                    Debug.Assert(!useContext);
                    Debug.Assert(IsAcceptState(D, false));
            ...
            if (previous.s0 != null)
            {
                ...
                switch (alts.Cardinality())
                {
                    ...
                    default:
                    {
                         ...
                         */
                        ATNConfigSet filteredConfigs = new();
        ...
        protected internal virtual SimulatorState ComputeReachSet(DFA dfa, SimulatorState previous, int t, PredictionContextCache contextCache)
        {
            ...
            if (useContext)
            {
                while (s.IsContextSymbol(t))
                {
                    ...
                    Debug.Assert(remainingGlobalContext != null);
                    remainingGlobalContext = (ParserRuleContext) remainingGlobalContext.Parent;
                    s = next;
                }
            }

            Debug.Assert(!IsAcceptState(s, useContext));
            ...
            Debug.Assert(!useContext || !target.configs.DipsIntoOuterContext);
        ...
        /// </returns>
        [return: NotNull]
        protected internal virtual Tuple<DFAState, ParserRuleContext> ComputeTargetState([NotNull] DFA dfa, [NotNull] DFAState s, ParserRuleContext remainingGlobalContext, int t,
            bool useContext, PredictionContextCache contextCache)
        {
            IList<ATNConfig> closureConfigs = new List<ATNConfig>(s.configs);
            List<int> contextElements = null;
            ATNConfigSet reach = new();
            bool stepIntoGlobal;
            do
            {
                ...
                ATNConfigSet reachIntermediate = new();
                ...
                foreach (ATNConfig c in closureConfigs)
                {
                    ...
                    {
                        Debug.Assert(c.Context.IsEmpty);
                ...
                {
                    Debug.Assert(skippedStopStates.Count > 0);
        ...
        {
            var pairs = new List<DFAState.PredPrediction>();
        ...
        protected internal virtual BitSet EvalSemanticContext([NotNull] DFAState.PredPrediction[] predPredictions, ParserRuleContext outerContext, bool complete)
        {
            ...
            foreach (DFAState.PredPrediction pair in predPredictions)
            {
                ...
                if (debug || dfa_debug)
                {
                    Console.Out.WriteLine("eval pred " + pair + "=" + evaluatedResult);
                }
                ...
                if (evaluatedResult)
                {
                    if (debug || dfa_debug)
                    {
                        Console.Out.WriteLine("PREDICT " + pair.alt);
                    }
        ...
        /* TODO: If we are doing predicates, there is no point in pursuing
              ...
              */
        protected internal virtual void Closure(ATNConfigSet sourceConfigs, [NotNull] ATNConfigSet configs, bool collectPredicates, bool hasMoreContext,
            [AllowNull] PredictionContextCache contextCache, bool treatEofAsEpsilon)
        {
        ...
        protected internal virtual void Closure([NotNull] ATNConfig config, [NotNull] ATNConfigSet configs, [AllowNull] ATNConfigSet intermediate,
            [NotNull] HashSet<ATNConfig> closureBusy, bool collectPredicates, bool hasMoreContexts, [NotNull] PredictionContextCache contextCache, int depth,
            ...
            if (config.State is RuleStopState)
            {
                ...
                if (!config.Context.IsEmpty)
                {
                        ...
                        i++)
                    {
                        ...
                        Debug.Assert(depth > Int32.MinValue);
                ...
                i_1++)
            {
                ...
                if (c != null)
                {
                    ...
                    if (config.State is RuleStopState)
                    {
                        ...
                        Debug.Assert(newDepth > Int32.MinValue);
                        newDepth--;
                    }
                    else
                    {
                        if (t is RuleTransition)
                        {
                            if (optimize_tail_calls && ((RuleTransition) t).optimizedTailCall && (!tail_call_preserves_sll || !PredictionContext.IsEmptyLocal(config.Context)))
                            {
                                Debug.Assert(c.Context == config.Context);
                                if (newDepth == 0)
                                {
        ...
        [return: MaybeNull]
        protected internal virtual ATNConfig GetEpsilonTarget([NotNull] ATNConfig config, [NotNull] Transition t, bool collectPredicates, bool inContext,
            PredictionContextCache contextCache, bool treatEofAsEpsilon)
        {
        ...
        [return: NotNull]
        protected internal virtual ATNConfig ActionTransition([NotNull] ATNConfig config, [NotNull] ActionTransition t)
        {
            return config.Transform(t.target, false);
        }
        ...
        [return: NotNull]
        protected internal virtual ATNConfig RuleTransition([NotNull] ATNConfig config, [NotNull] RuleTransition t, [AllowNull] PredictionContextCache contextCache)
        {
        ...
        private ConflictInfo IsConflicted([NotNull] ATNConfigSet configset, PredictionContextCache contextCache)
        {
            ...
            var configs = new List<ATNConfig>(configset);
            ...
            for (int i_1 = lastIndexCurrentStateMinAlt + 1;
                i_1 < configs.Count;
                i_1++)
            {
                ...
                if (state.NonStopStateNumber != currentState)
                {
                    ...
                    for (int j = firstIndexCurrentState + 1;
                        j < configs.Count;
                        j++)
                    {
                ...
                for (int j_1 = lastIndexCurrentStateCurrentAlt + 1;
                    j_1 < configs.Count;
                    j_1++)
                {
                ...
                if (c.State.NumberOfOptimizedTransitions > 0)
                {
                    ...
                    else
                    {
                        if (t is SetTransition)
                        {
                            ...
                            trans = (not ? "~" : String.Empty) + "Set " + st.set;
                        }
                    }
                }
        ...
        [return: NotNull]
        protected internal virtual NoViableAltException NoViableAlt([NotNull] ITokenStream input, [NotNull] ParserRuleContext outerContext, [NotNull] ATNConfigSet configs,
            int startIndex)
        {
        ...
        [return: NotNull]
        protected internal virtual DFAState AddDFAEdge([NotNull] DFA dfa, [NotNull] DFAState fromState, int t, List<int> contextTransitions, [NotNull] ATNConfigSet toConfigs,
            PredictionContextCache contextCache)
        {
            Debug.Assert(contextTransitions == null || contextTransitions.Count == 0 || dfa.IsContextSensitive);
            ...
            if (contextTransitions != null)
            {
                foreach (int context in contextTransitions.ToArray())
                {
                    ...
                    Debug.Assert(context != PredictionContext.EmptyFullStateKey || next.configs.IsOutermostConfigSet);
        ...
        protected internal virtual void AddDFAEdge([AllowNull] DFAState p, int t, [AllowNull] DFAState q)
        {
            if (p != null)
            {
                p.SetTarget(t, q);
        ...
        protected internal virtual DFAState AddDFAContextState([NotNull] DFA dfa, [NotNull] ATNConfigSet configs, int returnContext, PredictionContextCache contextCache)
        {
            ...
            {
                ATNConfigSet contextConfigs = new();
                foreach (ATNConfig config in configs)
                {
            ...
            Debug.Assert(!configs.IsOutermostConfigSet, "Shouldn't be adding a duplicate edge.");
        ...
        protected internal virtual DFAState AddDFAState([NotNull] DFA dfa, [NotNull] ATNConfigSet configs, PredictionContextCache contextCache)
        {
            ...
            {
                Console.Out.WriteLine("adding new DFA state: " + newState);
            }
        ...
        protected internal virtual void ReportAttemptingFullContext([NotNull] DFA dfa, [AllowNull] BitSet conflictingAlts, [NotNull] SimulatorState conflictState, int startIndex,
            int stopIndex)
        {
            if (debug || retry_debug)
            {
                Interval interval = Interval.Of(startIndex, stopIndex);
                Console.Out.WriteLine("reportAttemptingFullContext decision=" + dfa.decision + ":" + conflictState.s0.configs + ", input=" +
        ...
        protected internal virtual void ReportContextSensitivity([NotNull] DFA dfa, int prediction, [NotNull] SimulatorState acceptState, int startIndex, int stopIndex)
        {
            ...
            {
                Interval interval = Interval.Of(startIndex, stopIndex);
                Console.Out.WriteLine("reportContextSensitivity decision=" + dfa.decision + ":" + acceptState.s0.configs + ", input=" +
                                      ((ITokenStream) parser.InputStream).GetText(interval));
            }
        ...
        protected internal virtual void ReportAmbiguity([NotNull] DFA dfa, DFAState D, int startIndex, int stopIndex, bool exact, [NotNull] BitSet ambigAlts,
            [NotNull] ATNConfigSet configs)
        {
            ...
            {
                Interval interval = Interval.Of(startIndex, stopIndex);
                Console.Out.WriteLine("reportAmbiguity " + ambigAlts + ":" + configs + ", input=" + ((ITokenStream) parser.InputStream).GetText(interval));
            }
        ...
        protected internal ParserRuleContext SkipTailCalls(ParserRuleContext context)
        {
            ...
            {
                ATNState state = atn.states[context.invokingState];
                Debug.Assert(state.NumberOfTransitions == 1 && state.Transition(0).TransitionType == TransitionType.Rule);
                RuleTransition transition = (RuleTransition) state.Transition(0);
