...
namespace Antlr4.Runtime.Tree.Pattern
{
    ...
    public class ParseTreePatternMatcher
    {
        ...
        public virtual ParseTreePattern Compile(string pattern, int patternRuleIndex)
        {
            ...
            ListTokenSource tokenSrc = new(tokenList);
            CommonTokenStream tokens = new(tokenSrc);
            ParserInterpreter parserInterp = new(parser.GrammarFileName, parser.Vocabulary, Arrays.AsList(parser.RuleNames), parser.GetATNWithBypassAlts(), tokens);
            ...
            foreach (Chunk chunk in chunks)
            {
                if (chunk is TagChunk)
                {
                    ...
                    if (Char.IsUpper(tagChunk.Tag[0]))
                    {
                        ...
                        TokenTagToken t = new(tagChunk.Tag, ttype, tagChunk.Label);
                        tokens.Add(t);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    AntlrInputStream @in = new(textChunk.Text);
        ...
        internal virtual IList<Chunk> Split(string pattern)
        {
            ...
            if (ntags == 0)
            {
                string text = Utility.Runtime.Substring(pattern, 0, n);
            ...
            if (ntags > 0 && starts[0] > 0)
            {
                ...
                string text = Utility.Runtime.Substring(pattern, 0, starts[0]);
                chunks.Add(new TextChunk(text));
            }
            ...
            {
                // copy inside of <tag>
                string tag = Utility.Runtime.Substring(pattern, starts[i_1] + start.Length, stops[i_1]);
                string ruleOrToken = tag;
                string label = null;
                int colon = tag.IndexOf(':');
                if (colon >= 0)
                {
                    label = Utility.Runtime.Substring(tag, 0, colon);
                    ruleOrToken = Utility.Runtime.Substring(tag, colon + 1, tag.Length);
                }
                ...
                if (i_1 + 1 < ntags)
                {
                    ...
                    string text = Utility.Runtime.Substring(pattern, stops[i_1] + stop.Length, starts[i_1 + 1]);
                    chunks.Add(new TextChunk(text));
                }
            }
            ...
            if (ntags > 0)
            {
                ...
                if (afterLastTag < n)
                {
                    ...
                    string text = Utility.Runtime.Substring(pattern, afterLastTag, n);
                    chunks.Add(new TextChunk(text));
                }
            }
